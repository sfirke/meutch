...
- **Account security**: Signing up with an email address that somebody started to register but never confirmed now takes that address over, instead of telling you it is already taken. Before, your only way forward was to confirm that half-finished sign-up, which left whoever started it holding a working password for your account ([#499](https://github.com/sfirke/meutch/pull/499)).
