...
- Links in messages are now clickable in the conversation view and in message notification emails — previously a URL someone sent you had to be copied and pasted. Message text is also now properly escaped wherever it is displayed ([#472](https://github.com/sfirke/meutch/pull/472)).
- Links are now clickable wherever text you wrote is shown in full: request and item descriptions (home feed, detail pages, and public share pages), circle descriptions, profile bios, and the message attached to a giveaway request. URLs written without a scheme, like `www.example.com`, are linked too. In the home feed, long links are shortened so they do not crowd out the description ([#472](https://github.com/sfirke/meutch/pull/472)).
