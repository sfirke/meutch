...
- Updated Meutch's Python version and the software libraries it's built on, closing a batch of publicly known security flaws. Among them: a way to tie up the server with a specially crafted file upload, and a way to slip a hidden request past the load balancer. New and changed passwords are now stored with a stronger scrambling method, and existing passwords keep working as they are. Photo uploads now only accept the image types the site advertises (JPEG, PNG, GIF, BMP and WebP), so an unusual file type renamed to `.jpg` is turned away before it is ever opened.
...
- When you offer one of your items in answer to a request, the item's photo above the message box now shows in full instead of having its edges cut off ([#507](https://github.com/sfirke/meutch/pull/507)).
