...
- Borrowers can now ask for more time on a loan instead of having to talk the owner into changing the date. From the loan's message thread, propose a new due date and add a note; the owner gets an email and can approve or deny it from that same thread. Approving moves the due date and restarts the reminder emails. One open request per loan at a time ([#308](https://github.com/sfirke/meutch/pull/308)).

### Bug fixes
- When an item owner moves a loan's due date earlier, the borrower's email now says "Due Date Updated" rather than "Loan Request Approved", which is what it said before. Loan emails also no longer pick their subject line out of wording the sender typed themselves, so an owner writing something like "granting the extension requested last week" in their note is no longer filed as a brand new extension request ([#308](https://github.com/sfirke/meutch/pull/308)).
