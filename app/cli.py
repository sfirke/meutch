...
@click.command("init-system-user")
@with_appcontext
def init_system_user():
    """Create the reserved 'Meutch' system user (idempotent).

    This user is the sender for automated notifications such as circle
    join-request decisions, preserving admin anonymity.  It cannot log in,
    has no password, and is excluded from user lists throughout the app.
    """
    existing = db.session.get(User, SYSTEM_USER_ID)
    if existing:
        click.echo(f"ℹ️  System user already exists (id={SYSTEM_USER_ID}).")
        return
    ...
    # Set an unusable password so login is impossible even without the
    # explicit guard in authenticate_user().
    system_user.password_hash = "!"
    db.session.add(system_user)
    db.session.commit()
    click.echo(f"✅ System user created (id={SYSTEM_USER_ID}).")
...
def check_digest_sends_logic(now_utc=None, force_send=False):
    ...
    now_utc = _to_utc(now_utc) or datetime.now(UTC)
    users = User.query.filter_by(is_deleted=False).filter(User.id != SYSTEM_USER_ID).all()
