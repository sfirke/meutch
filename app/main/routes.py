...
from app.utils.home_feed import HOMEPAGE_FEED_EVENT_TYPES, build_homepage_feed_events
...
def _loan_conversion_blocker(item):
    ...
    active_interest = GiveawayInterest.query.filter(
        GiveawayInterest.item_id == item.id, GiveawayInterest.status.in_(["active", "selected"])
    ).first()
    if active_interest:
    ...
    if form.validate_on_submit():
        ...
        try:
            interest = GiveawayInterest(
                item_id=item.id, user_id=current_user.id, message=message_text, status="active"
            )
    ...
    if request.method == "POST":
        ...
        if selection_method == "first":
            ...
            selected_interest = GiveawayInterest.query.filter_by(
                item_id=item.id, user_id=manual_user_id, status="active"
            ).first()
...
def give_to_user(item_id, user_id):
    ...
    selected_interest = GiveawayInterest.query.filter_by(
        item_id=item.id, user_id=user_id, status="active"
    ).first()
...
def message_giveaway_requester(item_id, user_id):
    ...
    if form.validate_on_submit():
        # Create new message
        message = Message(
            sender_id=current_user.id, recipient_id=user_id, item_id=item.id, body=form.body.data
        )
        db.session.add(message)
...
def request_item(item_id):
    ...
    existing_request = LoanRequest.query.filter_by(
        item_id=item.id, borrower_id=current_user.id, status="pending"
    ).first()
...
def tag_items(tag_id):
    ...
    items_query = (
        ...
        .filter(
            ...
            User.vacation_mode.is_(False),  # Exclude vacation mode users
        )
    )
...
def profile():
    ...
    if search_query:
        my_items_search_filter = or_(
            Item.name.ilike(f"%{search_query}%"), Item.description.ilike(f"%{search_query}%")
        )
    ...
    active_giveaways_pagination = active_giveaways_query.order_by(Item.created_at.desc()).paginate(
        page=giveaway_page, per_page=per_page, error_out=False
    )
    ...
    past_giveaways_query = Item.query.filter_by(owner_id=current_user.id, is_giveaway=True).filter(
        Item.claim_status == "claimed", Item.claimed_at >= ninety_days_ago
    )
    if my_items_search_filter is not None:
    ...
    past_giveaways_pagination = past_giveaways_query.order_by(Item.claimed_at.desc()).paginate(
        page=past_giveaway_page, per_page=per_page, error_out=False
    )
...
def update_digest_settings():
    ...
    if not form.validate_on_submit():
        flash(
            "Unable to save digest settings. Please check your selections and try again.", "warning"
        )
...
def update_location():
    ...
    if not current_user.can_update_location():
        flash(
            "You can only update your location once per day. Please try again tomorrow.", "warning"
        )
        return redirect(url_for("main.profile"))
        ...
        if form.location_method.data == "remove":
            ...
            try:
                ...
                if coordinates:
                    ...
                    db.session.commit()
                    flash(
                        "We couldn't determine your location from that address. "
                        "You can try entering coordinates directly using the second option below.",
                        "warning",
                    )
                ...
                flash(
                    "There was an error determining your location from that address. "
                    "You can try entering coordinates directly using the second option below.",
                    "warning",
                )
            except Exception as e:
                ...
                flash(
                    "There was an error determining your location. "
                    "You can try entering coordinates directly using the second option below.",
                    "error",
                )
    ...
    latest_conversations = (
        ...
        .join(
            ...
            and_(
                ...
                or_(
                    and_(
                        Message.circle_id.is_(None), latest_messages_subquery.c.circle_id.is_(None)
                    ),
...
def view_conversation(message_id):
    ...
    if (
        ...
        active_interest = GiveawayInterest.query.filter_by(
            item_id=message.item.id, user_id=other_user.id, status="active"
        ).first()
        if active_interest:
            ...
            giveaway_selection_interested_count = GiveawayInterest.query.filter_by(
                item_id=message.item.id, status="active"
            ).count()
    ...
    if form.validate_on_submit():
        try:
            ...
            if current_user.vacation_mode:
                flash(
                    "Vacation mode enabled. Your items are now hidden from other users.", "success"
                )
            else:
                flash(
                    "Vacation mode disabled. Your items are now visible to other users.", "success"
                )
        except Exception:
            db.session.rollback()
