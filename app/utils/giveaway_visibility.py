...
# How many days claimed (rehomed) giveaways remain visible when OTHER users
# view a user's profile. Mirrors the 7-day RESOLVED_FEED_WINDOW_DAYS used by
# the home feed.
PROFILE_CLAIMED_GIVEAWAY_WINDOW_DAYS = 7

# How many days claimed (rehomed) giveaways remain visible in the owner's own
# "My Past Giveaways" section on /profile.
OWN_PROFILE_CLAIMED_GIVEAWAY_WINDOW_DAYS = 90
...
def can_view_claimed_giveaway(item, user, now=None, days=90):
    return is_giveaway_party(item, user) and is_claimed_giveaway_within_visibility_window(
        item, now=now, days=days
    )
    ...
    if is_authenticated:
        ...
        query = Item.query.join(User, Item.owner_id == User.id).filter(
            ...
            or_(
                ...
                and_(Item.giveaway_visibility == "public", Item.owner_id.in_(all_circle_user_ids)),
            ),
        )
    else:
        showcase_user_ids = select(User.id).where(
            User.is_public_showcase.is_(True), User.vacation_mode.is_(False)
        )
