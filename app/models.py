...
class User(UserMixin, db.Model):
    ...
    def distance_to(self, other_user):
        ...
        return calculate_distance(
            self.latitude, self.longitude, other_user.latitude, other_user.longitude
        )

    def shared_circles_with(self, other_user):
        """Return shared circles with another user in a stable display order."""
        if not other_user:
            return []

        other_circle_ids = {circle.id for circle in other_user.circles}
        shared_circles = [circle for circle in self.circles if circle.id in other_circle_ids]
        return sorted(shared_circles, key=lambda circle: circle.name.casefold())
    ...
    def get_shared_circle_user_ids_query(self):
        ...
        return (
            ...
            .where(circle_members.c.circle_id.in_(user_circle_ids))
            .distinct()
        )
    ...
    def can_update_location(self):
        ...
        geocoded_at_utc = (
            self.geocoded_at.replace(tzinfo=UTC)
            if self.geocoded_at.tzinfo is None
            else self.geocoded_at
        )
    ...
    def confirm_email(self, token):
        ...
        if self.email_confirmation_token and secrets.compare_digest(
    ...
    def reset_password(self, token, new_password):
        ...
        if self.password_reset_token and secrets.compare_digest(self.password_reset_token, token):
            ...
            if self.password_reset_sent_at:
                ...
                reset_sent_at_utc = (
                    self.password_reset_sent_at.replace(tzinfo=UTC)
                    if self.password_reset_sent_at.tzinfo is None
                    else self.password_reset_sent_at
                )
    ...
    def get_outstanding_loans_summary(self):
        ...
        pending_borrowing = LoanRequest.query.filter_by(
            borrower_id=self.id, status="pending"
        ).count()

        # Pending requests as owner
        pending_lending = (
            LoanRequest.query.join(Item)
            .filter(Item.owner_id == self.id, LoanRequest.status == "pending")
            .count()
        )
    ...
    def delete_account(self):
        ...
        LoanRequest.query.filter_by(borrower_id=self.id, status="pending").update(
            {"status": "canceled"}
        )
        ...
        if item_ids:
            LoanRequest.query.filter(
                LoanRequest.item_id.in_(item_ids), LoanRequest.status == "pending"
...
class Item(db.Model):
    ...
    def current_loan(self):
        return (
            LoanRequest.query.filter_by(item_id=self.id, status="approved")
            .order_by(LoanRequest.end_date.desc())
            .first()
        )
...
class ItemImage(db.Model):
    ...
    item_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("item.id", ondelete="CASCADE"), nullable=False
    )
...
class GiveawayInterest(db.Model):
    ...
    def __repr__(self):
        return (
            f"<GiveawayInterest item_id={self.item_id} user_id={self.user_id} status={self.status}>"
        )
...
class LoanRequest(db.Model):
    ...
    )  # pending, approved, canceled, denied, completed
    created_at = db.Column(db.DateTime, default=func.now())
...
class Feedback(db.Model):
    ...
    loan_request_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("loan_request.id"), nullable=False
    )
...
class ItemRequest(db.Model):
    ...
    def show_in_feed(self):
        ...
        if self.status == "fulfilled" and self.fulfilled_at:
            fulfilled_at_utc = (
                self.fulfilled_at.replace(tzinfo=UTC)
                if self.fulfilled_at.tzinfo is None
                else self.fulfilled_at
            )
