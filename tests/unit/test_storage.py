...
class TestImageProcessing:
    ...
    def test_process_image_jpeg(self, mock_transpose, mock_open, app):
        ...
        with app.app_context():
            ...
            mock_open.assert_called_once_with(mock_file, formats=ALLOWED_IMAGE_FORMATS)
    ...
    @pytest.mark.parametrize("pil_format", ["JPEG", "PNG", "GIF", "BMP", "WEBP"])
    def test_process_image_accepts_allowed_formats(self, pil_format, app):
        """Every format behind an allowed upload extension still decodes."""
        source = BytesIO()
        Image.new("RGB", (40, 30), (200, 100, 50)).save(source, format=pil_format)
        source.seek(0)
    ...
    def test_process_image_accepts_multi_picture_jpeg(self, app):
        """Phone cameras can write MPO (multi-picture) JPEGs; the JPEG decoder reads them."""
        source = BytesIO()
        Image.new("RGB", (40, 30)).save(
            source, format="MPO", save_all=True, append_images=[Image.new("RGB", (40, 30))]
        )
        source.seek(0)

        with app.app_context():
            assert process_image(source) is not None

    def test_process_image_rejects_other_formats(self, app):
        """A format Pillow can read but uploads don't allow is refused, even when
        it would decode fine (e.g. a TIFF renamed to .jpg)."""
        source = BytesIO()
        Image.new("RGB", (40, 30)).save(source, format="TIFF")
        source.seek(0)

        with app.app_context():
            assert process_image(source) is None
...
class TestUploadFunctions:
    ...
    def test_upload_item_image(self, mock_upload):
        ...
        mock_upload.assert_called_once_with(
            mock_file, folder="items", max_width=800, max_height=600, quality=85
        )
    ...
    def test_upload_profile_image(self, mock_upload):
        ...
        mock_upload.assert_called_once_with(
            mock_file, folder="profiles", max_width=400, max_height=400, quality=90
        )
    ...
    def test_do_spaces_upload(self, mock_boto_client, app):
        ...
        with app.app_context():
            ...
            storage = DOSpacesStorage(
                region="nyc3", key="test-key", secret="test-secret", bucket="test-bucket"
            )
            ...
            assert (
                url
                == "https://test-bucket.nyc3.cdn.digitaloceanspaces.com/test-folder/test-file.jpg"
            )
    ...
    def test_do_spaces_delete(self, mock_boto_client, app):
        ...
        with app.app_context():
            ...
            mock_s3.delete_object.assert_called_once_with(
                Bucket="test-bucket", Key="test-folder/test-file.jpg"
            )
