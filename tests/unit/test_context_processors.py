...
class TestUnreadMessagesCount:
    ...
    def test_inject_unread_messages_count_excludes_self_sent(self, app):
        ...
        with app.app_context():
            ...
            MessageFactory(sender=user, recipient=user, conversation=conversation, is_read=False)
    ...
    def test_inject_unread_messages_count_excludes_pending_circle_requests_for_admin(self, app):
        ...
        with app.app_context():
            ...
            db.session.execute(
                circle_members.insert().values(user_id=admin.id, circle_id=circle.id, is_admin=True)
            )
            db.session.commit()
    ...
    def test_inject_unread_messages_count_request_messages(self, app):
        ...
        with app.app_context():
            ...
            req_conversation = ConversationFactory(
                context_type="request", context_id=item_request.id
            )

            # Create request conversation message (unread) - no item, just request
            MessageFactory(
                sender=sender, recipient=recipient, conversation=req_conversation, is_read=False
            )
            ...
            MessageFactory(
                sender=sender, recipient=recipient, conversation=req_conversation, is_read=True
            )
...
class TestDistanceUtils:
    ...
    def test_get_distance_to_circle_exception_handling(self, app):
        ...
        with app.app_context():
            ...
            with patch("app.context_processors.current_user", mock_user):
                with patch.object(
                    circle, "distance_to_user", side_effect=Exception("Calculation error")
                ):
                    context = inject_distance_utils()
                    get_distance = context["get_distance_to_circle"]
