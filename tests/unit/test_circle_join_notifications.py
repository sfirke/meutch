...
class TestCircleJoinRequestNotifications:
    ...
    def test_circle_name_html_is_escaped_in_decision_email(self, app):
        ...
        with app.app_context():
            # Create test users and circle with HTML in name
            requesting_user = UserFactory(first_name="Bob", last_name="User")
            circle = CircleFactory(name="Test <script>Circle</script>")
            ...
            with patch("app.utils.email.send_email") as mock_send_email:
                ...
                # HTML content should have escaped circle name
                assert "Test &lt;script&gt;Circle&lt;/script&gt;" in html_content
                # Raw script tag should NOT be present in HTML
                assert "<script>Circle</script>" not in html_content
