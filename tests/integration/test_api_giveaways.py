...
class TestApiGiveawayInterestMutations:
    ...
    def test_express_interest_creates_active_interest_and_returns_item_state(self, client, app):
        with app.app_context():
            owner = UserFactory()
        ...
        response = client.post(
            ...
            headers=auth_headers(access_token),
            json={"message": "I can pick this up tonight."},
        )

        assert response.status_code == 201
        payload = response.get_json()

        assert payload["interest"]["status"] == "active"
        assert payload["interest"]["user"]["id"] == requester_id_str
        assert payload["item"]["id"] == item_id_str
        assert payload["item"]["viewer_interest_status"] == "active"
        assert payload["item"]["claim_status"] == "unclaimed"

        with app.app_context():
            interest = GiveawayInterest.query.filter_by(item_id=item_id, user_id=requester_id).one()
            assert interest.message == "I can pick this up tonight."

    def test_owner_cannot_express_interest_in_own_giveaway(self, client, app):
        with app.app_context():
            owner = UserFactory(email_confirmed=True)
            item = ItemFactory(
                owner=owner,
        ...
        response = client.post(
            f"/api/v1/items/{item_id}/interest",
            headers=auth_headers(access_token),
            json={"message": "Keeping this for myself."},
        )

        assert response.status_code == 409
        assert response.get_json()["error"]["code"] == "CONFLICT"

    def test_express_interest_rejects_non_giveaway_item(self, client, app):
        with app.app_context():
            owner = UserFactory()
            requester = UserFactory(email_confirmed=True)
