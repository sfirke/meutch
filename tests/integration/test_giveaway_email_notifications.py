    ...
    def test_email_sent_on_random_selection(self, client, app, auth_user):
        ...
        with app.app_context():
            ...
            for user in [requester1, requester2]:
                interest = GiveawayInterest(item_id=giveaway.id, user_id=user.id, status="active")
                db.session.add(interest)
            db.session.commit()
            ...
            with patch(
                ...
                with patch(
                    "random.choice",
                    return_value=GiveawayInterest.query.filter_by(
    ...
    def test_email_sent_to_new_recipient_on_reassignment(self, client, app, auth_user):
        ...
        with app.app_context():
            ...
            prev_interest = GiveawayInterest(
                item_id=giveaway.id, user_id=previous_recipient.id, status="selected"
            )
            db.session.add(prev_interest)

            # New recipient's interest - still active
            new_interest = GiveawayInterest(
                item_id=giveaway.id, user_id=new_recipient.id, status="active"
            )
        ...
        with app.app_context():
            ...
            with patch(
                ...
                assert (
                    ...
                    or "selected" in prev_message.body.lower()
                )
        ...
        with app.app_context():
            ...
            with patch(
                ...
                response = client.post(
                    ...
                    data={"message": "I would love this item!"},
                    follow_redirects=True,
                )
    ...
    def test_no_email_sent_to_other_interested_users_on_release(self, client, app, auth_user):
        ...
        with app.app_context():
            ...
            other_interest = GiveawayInterest(
                item_id=giveaway.id, user_id=other_user.id, status="active"
            )
            ...
            with patch(
                ...
                release_messages_to_other = [
                    m
                    for m in messages
                    if m.recipient_id == other_user.id and "released" in m.body.lower()
                ]
...
class TestGiveawayHandoffNoEmail:
    ...
    def test_no_email_on_confirm_handoff(self, client, app, auth_user):
        ...
        with app.app_context():
            ...
            interest = GiveawayInterest(
                item_id=giveaway.id, user_id=recipient.id, status="selected"
            )
            db.session.add(interest)
            db.session.commit()
            ...
            with patch(
                ...
                response = client.post(
                    f"/item/{giveaway.id}/confirm-handoff", follow_redirects=True
                )
