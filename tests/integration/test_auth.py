...
class TestAuthenticationRoutes:
    ...
    def test_login_with_remember_device_sets_cookie(self, client, app, auth_user):
        ...
        with app.app_context():
            ...
            response = client.post(
                "/login",
                data={"email": user.email, "password": TEST_PASSWORD, "remember_device": "y"},
    ...
    def test_register_page_form_action_preserves_next(self, client):
        ...
        assert (
            b'action="/register?next=%2Fshare%2Fgiveaway%2Fabc123"' in response.data
            or b'action="/register?next=/share/giveaway/abc123"' in response.data
        )
    ...
    def test_case_insensitive_resend_confirmation(self, client, app):
        ...
        with app.app_context():
            ...
            UserFactory(email="confirm@example.com", email_confirmed=False)
...
class TestEmailConfirmation:
    ...
    def test_resend_confirmation_valid_email(self, client, app):
        ...
        with app.app_context():
            ...
            response = client.post(
                "/resend-confirmation", data={"email": user.email}, follow_redirects=True
            )
    ...
    def test_resend_confirmation_already_confirmed(self, client, app):
        ...
        with app.app_context():
            ...
            response = client.post("/resend-confirmation", data={"email": user.email})
...
class TestPasswordReset:
    ...
    def test_forgot_password_invalid_email(self, client):
        ...
        response = client.post("/forgot-password", data={"email": "nonexistent@example.com"})
...
class TestRedirectAfterLogin:
    ...
    def test_login_message_shown_on_unauthorized_access(self, client, app):
        ...
        with app.app_context():
            ...
            assert (
                b"Please log in to access this page" in response.data or b"Log In" in response.data
            )
