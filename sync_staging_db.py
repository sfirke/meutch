...
def sync_staging_db():
    ...
    if verify_result.returncode == 0:
        ...
        click.echo(
            f"   ⚠️  Could not verify: {verify_result.stderr.splitlines()[0] if verify_result.stderr else 'Unknown error'}"
        )
    ...
    try:
        ...
        # Clear JWT token tables synced from production.
        # JWTs are bearer tokens with no domain binding — unlike session cookies they are
        # not scope-limited to the environment that issued them.  Staging uses a different
